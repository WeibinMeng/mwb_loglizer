...
def evaluate(event_count_matrix, invar_dict, groundtruth_labels):
		...
		for i, cols in enumerate(valid_col_list):
			...
			for j, c in enumerate(cols):
				...
				if row[c]!=0:
					# test_n+=1
					test_rowc=1
				# 	# print(str(j)+' sum_of_invar',str(sum_of_invar),'valid_invar_list[i][j]',str(valid_invar_list[i][j]),'row[c]',str(row[c]))
			# if sum_of_invar==0 and test_rowc ==1:
			# 	print 'sssssss'
	...
	# # #test1
	# print('test1')
	# for index,row in enumerate(event_count_matrix):
	# 	if prediction[index]==0:
	# 		for i, cols in enumerate(valid_col_list):
	# 			# print(type(cols),len(cols))
	# 			if row[cols[0]]!=0 or row[cols[1]]!=0:
	# 				# print('')
	# 				print ('row[cols[0]],row[cols[1]]',row[cols[0]],row[cols[1]],'valid_invar:',valid_invar_list[i])
		
	# #test2
	# print('test2')
	# for t, t_label in enumerate(prediction):
	# 	if t_label==0:
	# 		row=event_count_matrix[t]
	# 		for i, cols in enumerate(valid_col_list):
	# 			sum_of_invar = 0
	# 			for j, c in enumerate(cols):
	# 				sum_of_invar += valid_invar_list[i][j] * row[c]
	# 				if row[c]!=0:
	# 					sum_of_invar1 = 0
	# 					for j1, c1 in enumerate(cols):
	# 						sum_of_invar1 += valid_invar_list[i][j] * row[c1]
	# 						print(str(j1)+' sum_of_invar',str(sum_of_invar1),'valid_invar_list[i][j]',str(valid_invar_list[i][j1]),'row['+str(c1)+']',str(row[c1]))
	# 					print ('')
	# 				break

	# test_l=[a for a in prediction if a==1]
	# print(len(test_l),str(test_n),test_nomal)
...
def detect(para,event_count_matrix, invar_dict, windows_time_list):
	""" evaluate the results with mined invariants
	...
	invar_dict: the dictionary of invariants
	groundtruth_labels: the groundtruth labels for evaluation
	windows_time_list: [(start,end),(start,end)]
	Returns:
	--------
	"""
	print("the mined {} invariants are: {}".format(len(invar_dict), invar_dict))
	...
	print('# of total windows:',len(prediction))
	print('# of anomalous windows:',sum(prediction))
	f=file( para['path'] + para['prediction_file'],'w')
	for t,k in zip(windows_time_list, prediction):
		f.writelines(str(k)+'\n')
